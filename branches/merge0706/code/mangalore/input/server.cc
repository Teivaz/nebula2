...
Server::Open()
{
    ...
    // make shure the expected mappings exist to keep track of the mouse position
...
Server::Trigger()
{
    ...
    // try to cleanup empty mapping elememts
...
Server::CreateInputSource(const nString& deviceChannel) const
{
    ...
    if (isDefaultKeyboard)
    {
        if (isCharacterInput)
        {
            // the default text/character input source (used to pipe the raw characters from win proc into textfield)
