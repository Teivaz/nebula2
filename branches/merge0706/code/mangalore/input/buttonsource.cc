...
ButtonSource::ButtonUp(const vector2& mousePosition, bool hasMousePosition, Event::Button button)
{
    ...
    if (this->buttonDownEvents.Size() == this->buttonUpEvents.Size())
    {
        ...
        // connect the up event to the clicked event (!! this create a circular PTR refernce !! to relase this use Disconnect on the event)
    ...
    {
        // ??? Don't know if this is realy true but is seem like windows is doing so
