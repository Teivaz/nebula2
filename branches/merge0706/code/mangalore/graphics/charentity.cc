...
CharEntity::UpdateRenderContextVariables()
{
    ...
    if (this->nebCharacter)
    {
        ...
        // communicate restart anim flag to Nebula
...
CharEntity::OnRenderBefore()
{
    ...
    if (this->nebCharacter)
    {
        if (this->IsOverlayAnimationActive())
        {
            // HACK: overlay anim needs to be stopped slightly before it's computed end time to avoid plopping
            static const nTime OverlayAnimStopBuffer = 0.3; // to assert overlay stopping before anim has finished
...
CharEntity::LoadCharacter3Set(const nString& fileName)
{
    ...
    // set new character set in render contex
