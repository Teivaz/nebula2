...
nChunkLodTree::LoadResource()
{
    ...
    this->texQuadTree = n_new(nTextureQuadTree(kernelServer, nGfxServer2::Instance()));
    ...
    if (!this->chuFile->Open(this->GetFilename().Get(), "rb"))
    {
    ...
    // format version
    int formatVersion = this->chuFile->GetShort();
    if (formatVersion != 9)
    {
    ...
    this->treeDepth          = this->chuFile->GetShort();
    this->errorLodMax        = this->chuFile->GetFloat();
    this->verticalScale      = this->chuFile->GetFloat() * this->terrainScale;
    this->baseChunkDimension = this->chuFile->GetFloat() * this->terrainScale;
    this->chunkCount         = this->chuFile->GetInt();
...
{
    /*
        FIXME: should no longer be necessary, since transform
        shader params are now shared!

    if (shader->IsParameterUsed(nShader2::ModelViewProjection))
    {
        shader->SetMatrix(nShader2::ModelViewProjection, modelViewProj);
    }

    if (shader->IsParameterUsed(nShader2::Projection))
    {
        shader->SetMatrix(nShader2::Projection, projection);
    }
    */
