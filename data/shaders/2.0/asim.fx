...
vsOutputStaticColor vsUnderwater(const vsInputStaticColor vsIn)
{
    ...
    //float modTime = fmod(Time, 100.0f);
    //float scale = 1.0 + WaterMovement(amplitude, periode);
...
//------------------------------------------------------------------------------
/**
    Pixel shader for underwater surface
*/
color4 psUnderwater(const vsOutputStaticColor psIn) : COLOR
{
    color4 diffColor;
    color4 tex = tex2D(DiffSampler, psIn.uv0);
    diffColor = MatAmbient;
    diffColor.a = tex.a;
    return diffColor;
};
...
//------------------------------------------------------------------------------
/**
    Techniques for shader "underwaterground"
*/
...
technique tUnderwater
{
    pass p0
    {
        ...
        VertexShader = compile VS_PROFILE vsUnderwater();
        PixelShader  = compile PS_PROFILE psUnderwater();
    }
}
