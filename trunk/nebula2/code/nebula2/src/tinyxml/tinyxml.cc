...
bool TiXmlDocument::LoadFile( const char* filename )
{
    ...
    if (file->Open(value.c_str(), "r"))
    {
        ...
        // There is a problem, when the header(name + attributes) of an object is bigger than the BUF_SIZE.
        // It doesn't get parsed correctly, so I set it to 8KB instead of 2KB.
