...
/**
   ...
   Various subclasses of this abstract class are used for specific types of visibility culling.  To add
   a new type of visibility visitor:

   1. Subclass this class.  Add in stacks for any data that depends on the coordinate system.

   2. override the Accept(nSpatialSector *visitee, int recursedepth) method to correctly cull the octree
   for that sector.  Invoke Visit() for each element you find visible.

   3. To make heirarchy objects work, override VisibilityTest(...) to perform a proper test for the given element.

   4. To handle sectors with different coordinate systems, override the Enter/LeaveLocalSpace methods, such that
   you transform into the sectors space and push the new state onto your stacks.

   5. In your class or a subclass, override Accept(nSpatialElement *, int) to process each visible element, either by accumulating the
   visible elements in a list, sending them to a render system, or whatever.
*/
...
public:
    ...
    /** Check if an element should be culled.
        Returns true if the element is (possibly) visible, false if it is not visible
    ...
    /** @brief get the current viewpoint in local sector space
        Mainly used by hiearchy elements; needed for occlusion or LOD testing */
