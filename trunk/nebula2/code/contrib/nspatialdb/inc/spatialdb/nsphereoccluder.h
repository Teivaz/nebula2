...
/**
    ...
    test object that does not intersect the sphere.

    The return value of an occlusion test is a two-member @c result_info,
    which is designed for efficient use by hierarchical objects.  In many
    spatial hierarchies it holds true that if a specific tree node is
    fully occluded/not occluded then all the children of that node are
    also occluded/not occluded, and so we can avoid doing additional tests
    on the nodes children when we already know what the answer will be.
...
nSphereOccluder::result_info nSphereOccluder::TestSphere(const sphere &spheretest, nSphereOccluder::result_info ri)
{
    ...
    // trivial culling criteria: distance from occluder to viewpoint is
    // less than the occluder radius
    if (occluderline.len() < m_sphere.r)
        return nSphereOccluder::result_info(true,0);

    // if the test sphere is completely in front of the occluder, we have
    // a trivial non-occlusion
    ...
    // find the distance between the center of the test sphere and the
    // line projected from the viewpoint to the occluder's center
    float testdistance = occluderline.distance(spheretest.p);

    // at this point, you pretend that the occluder has been moved out to
    // the same distance from the viewpoint as the test sphere along its
    // occluder line, with an appropriate increase in radius. now, does
    // the occluder totally swallow up the sphere?  we have to check the
    // test sphere's radius and the distance of the test sphere from the line.
    if ( occludertestradius > testdistance + spheretest.r )
        return nSphereOccluder::result_info(true,0);

    // ok, maybe the test sphere is so far from the line that the occluder
    // doesn't enclose it at all. in this case, we have definite non-occlusion
    if ( occludertestradius < testdistance - spheretest.r )
        return nSphereOccluder::result_info(false,0);
     
    // can't determine visibility, it's a partial occlusion.  just
    // return the result_info that was passed in
    return ri;
}
