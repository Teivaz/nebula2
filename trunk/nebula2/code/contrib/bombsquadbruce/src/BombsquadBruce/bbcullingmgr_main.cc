...
nNebulaScriptClass(BBCullingMgr, "nroot");
...
void BBCullingMgr::DestroySpatialDB(nSpatialSector *cleanme)
{
    int i;
...
void BBCullingMgr::StartElements()
{
    if( m_rRootSector.isvalid() )
...
bool BBCullingMgr::AddElement(BBRoot& root)
{
...
void BBCullingMgr::EndElements()
{
    //m_rRootSector->BalanceTree();
}

const nVisibilityVisitor::VisibleElements& BBCullingMgr::GetVisibleElements(const BBCamera& cullCamera)
{
    n_delete( m_pCuller );
