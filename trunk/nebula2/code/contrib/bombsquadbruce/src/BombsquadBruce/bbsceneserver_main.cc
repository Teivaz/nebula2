...
nVariable::Handle BBSceneServer::m_AlphaFlagHandle = nVariable::InvalidHandle;
//------------------------------------------------------------------------------
/**
*/
BBSceneServer::BBSceneServer( ) 
{
    n_assert2( !bbself, "There can be only one BBSceneServer!" );
    bbself = this;
...
//------------------------------------------------------------------------------
nVariable::Handle BBSceneServer::GetLODVarHandle(int i) const 
{ 
...
*/
void
BBSceneServer::UpdateShader(nShader2* shd, nRenderContext* renderContext)
{
...
*/
void
BBSceneServer::DoSort( ushort* indexPtr, int numIndices )
{
    qsort(indexPtr, numIndices, sizeof(ushort), (int(__cdecl *)(const void *, const void *)) BBCompare);
}
//------------------------------------------------------------------------------
/**
...
int
__cdecl
BBSceneServer::BBCompare(const ushort* i1, const ushort* i2)
{
    BBSceneServer* sceneServer = BBSceneServer::bbself;
...
*/  
void
BBSceneServer::SplitNodes(uint shaderFourCC)
...
*/
//void
//BBSceneServer::RenderLightShapes(uint shaderFourCC)
//{
...
bool BBSceneServer::SetLOD( Group& shapeGroup )
{
    bool retVal = true;
...
void BBSceneServer::SubInLowPolyModel( Group& shapeGroup, int LODShapeIndex )
{
...
void BBSceneServer::SubInBillboard( Group& shapeGroup, int LODShapeIndex )
{   // substitute a billboard image (whose nShapeNode is stored in 
    ...
    billboardMatrix.billboard( pos, BBUtil::UP_VECTOR ); 
