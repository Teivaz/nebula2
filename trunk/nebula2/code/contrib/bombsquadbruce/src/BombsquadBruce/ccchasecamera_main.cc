...
nNebulaScriptClass(CCChaseCamera, "cccamera");

//------------------------------------------------------------------------------
CCChaseCamera::CCChaseCamera() :
	m_Mode(NONE),
...
/**
    ...
    button is held down.
*/
void CCChaseCamera::SetSpinMode(SpinMode mode)
{
    m_Mode = mode;
}

void CCChaseCamera::SetMaxZoomDist(float maxZoomDist )
{
    if( maxZoomDist >= 0.0f )
    {
...
*/
void CCChaseCamera::Trigger(double deltaTime)
{
...
*/
void CCChaseCamera::ReactToInput()
{
	Move();
    Zoom();
	switch( m_Mode )
...
void CCChaseCamera::Chase(double deltaTime)
{
    vector3 newPos;
    ...
    m_TransformMatrix.lookatRh( newPos, CCUtil::UP_VECTOR );    
}
...
void CCChaseCamera::DoZoom( float zoomDelta )
{
...
void CCChaseCamera::SpinAround( vector3& orbitThisPt, const vector3& aroundThatPt, float speed )
{
...
*/
void CCChaseCamera::AdjustPositionInScript(vector3& posToAdjust, double deltaTime)
{   
