...
/**
    ...
    How this instructions are interpreted is left to CCCamera's
...
// TODO Refactor this into CCInputCamera (an nRoot that only handles input)
// CCCullingCamera (a child of stripped down CCRoot (see below) and nCamera2
// CCCamera will be a child of those two.
// a mixin should handle the Manipulate stuff, perhaps; otherwise, it goes in CCCullingCamera 
//------------------------------------------------------------------------------
class nInputServer;
class nGfxServer2;

class CCCamera : public nRoot, public nCamera2
{
public:
    CCCamera();
    virtual ~CCCamera();
    ...
    void Attach( nSceneServer& sceneServer, uint& frameId ); // TODO: get rid of this -- make it descend from CCRoot
    /// Returns an initialized, camera-dependent subclass of nVisibilityVistor, for use by CCCullingMgr, which owns the returned pointer.
