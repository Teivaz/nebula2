#ifndef BB_CHASECAMERA_H
#define BB_CHASECAMERA_H
//------------------------------------------------------------------------------
/**
    @class BBChaseCamera
    @ingroup BombsquadBruceContribModule
    @brief Follows a specified target BBRoot around.

    BBChaseCamera maintains its position at a (script-settable) offset
    relative to a (script-settable) BBRoot target, and looks at a focal
...
class BBChaseCamera : public BBCamera
{
public:
	enum SpinMode 
...
public:
    ...
    // nonscript commands:
    /// Update the camera's position and orientation based on user input, mode, and target BBRoot's transformation.
...
private:
    ...
    nDynAutoRef<BBRoot> m_TargetEntity;
    SpinMode m_Mode;
