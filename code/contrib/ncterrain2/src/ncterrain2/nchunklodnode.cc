...
*/
bool
nChunkLodNode::LoadParams(nFile* file, int recurseCount, nChunkLodTree* tree)
{
    n_assert(file);
    n_assert(tree);

    // read the chunk's label
    int chunkLabel = file->GetInt();
    ...
    for (i = 0; i < 4; i++)
    {
        this->neighbour[i].label = file->GetInt();
    }

    // read chunk address
    this->level = file->GetChar();
    this->x = file->GetUShort();
    this->z = file->GetUShort();
    this->minY = file->GetShort();
    this->maxY = file->GetShort();

    // read vertex data file offset
    this->vertexDataPosition = file->GetInt();
...
nChunkLodNode::GetTexGen(nChunkLodTree* tree, nFloat4& texGenS, nFloat4& texGenT) const
{
    ...
    float xSize = extent.x * 2.0f;
    float zSize = extent.z * 2.0f;
    float x0 = center.x - extent.x - ((xSize / 256.0f) * 0.5f);
    float z0 = center.z - extent.z - ((zSize / 256.0f) * 0.5f);
...
nChunkLodNode::RenderTexture(nChunkLodRenderParams& renderParams)
{
    ...
    if (renderParams.shader->IsParameterUsed(nShaderState::TextureTransform0))
    {
        ...
        matrix44 invModelView = gfxServer->GetTransform(nGfxServer2::InvModelView);
