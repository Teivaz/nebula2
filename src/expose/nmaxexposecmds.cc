...
TCHAR* nNebulaInterface::New(TCHAR *className, TCHAR *objectName)
{
    ...
    if (!className)
    {
        n_listener ("NULL classname.\n");
        return _T("");
    }
    if (!objectName)
    {
        n_listener ("NULL objectname.\n");
        return _T("");
    }
...
BOOL nNebulaInterface::Delete(TCHAR* objectName)
{
    ...
    {
        n_listener("Could not lookup object '%s'\n", objName);
        return FALSE;
    }
...
TCHAR* nNebulaInterface::Sel_(TCHAR* objectName, bool setCwd)
{
    ...
    if (*objName == '\0')
    {
        n_listener ("NULL object name is used.\n");
        return _T("");
    }
    ...
    {
        n_listener("SEL: Unable to select path (perhaps it doesn't exist)\n");
        return _T("");
    }
        ...
        if (*commandName != '\0')
        {
            ...
            if (o)
            {
                ...
                if (cmdProto)
                {
                    ...
                    {
                        n_listener ("Dispatch error, object '%s', command '%s'\n",
                                    o->GetName(), commandName);
                    }
                ...
                else
                {
                    ...
                    n_listener ("Unknown command, object '%s', command '%s'\n",
                                o->GetName(), commandName);
                }
            }
            else
            {
                n_listener ("Unable to acruire current object.\n");
            }
        }
        else
        {
            n_listener ("No command.\n"); // command is NULL
        }
    }
    else
    {
        // Unable to acquire command string.
        n_listener ("Unable to acquire command string.\n");
    }
    ...
    if (numArgs == val->Count())
    {
        ...
        for (int argIndex=0; argIndex<numArgs; argIndex++)
        {
            ...
            case nArg::Type::Int:
                {
                    if (v->type != TYPE_INT)
                        n_listener("WARNING: argument '%d' and command argument [int] is diffferent type.\n", argIndex);
            ...
            case nArg::Type::Float:
                {
                    if (v->type != TYPE_FLOAT)
                        n_listener("WARNING: argument '%d' and command argument [string] is diffferent type.\n", argIndex);
            ...
            case nArg::Type::Bool:
                {
                    if (v->type != TYPE_BOOL)
                        n_listener("WARNING: argument '%d' and command argument [bool] is diffferent type.\n", argIndex);
            ...
            case nArg::Type::Object:
                n_listener ("Object values aren't acceptable in arguments.\n");
                argOk = false;
                break;

            case nArg::Type::List:
                n_listener ("List values aren't acceptable in arguments.\n");
                argOk = false;
                break;
