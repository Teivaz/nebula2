...
nD3D9Server::DrawShapePrimitives(PrimitiveType type, int numPrimitives, const vector3* vertexList, int vertexWidth, const matrix44& model, const vector4& color)
{
    ...
    this->PushTransform(nGfxServer2::Model, model);
    shd->BeginPass(0);
    HRESULT hr = this->d3d9Device->DrawPrimitiveUP(d3dPrimType, numPrimitives, vertexList, vertexWidth * 4);
                                        ...
                                        const vector4& color)
{
    ...
    this->PushTransform(nGfxServer2::Model, model);
    shd->BeginPass(0);
    HRESULT hr = this->d3d9Device->DrawIndexedPrimitiveUP(d3dPrimType,      // PrimitiveType
                                                          0,                // MinVertexIndex
