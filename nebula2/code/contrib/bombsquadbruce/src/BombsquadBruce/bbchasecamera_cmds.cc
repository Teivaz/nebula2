...
/**
    ...
    @classinfo
    A BBChaseCameras is entity-centric: it follows a given BBRoot entity
...
{
    BBChaseCamera* self = (BBChaseCamera*)slf;
...
n_setspinmode(void* slf, nCmd* cmd)
{
	...
	if(modeStr=="NONE")
	{
        self->SetSpinMode( BBChaseCamera::NONE );
	}
    else if(modeStr=="ORBIT")
	{
		self->SetSpinMode( BBChaseCamera::ORBIT );
	}
	else if(modeStr=="ROTATE")
	{
		self->SetSpinMode( BBChaseCamera::ROTATE );
	}
    else 
...
*/
bool
BBChaseCamera::SaveCmds(nPersistServer* ps)
{
