...
nNebulaScriptClass(BBCamera, "nroot");
...
BBCamera::~BBCamera()
{
    if( m_RenderContext.IsValid() )
        m_RenderContext.GetRootNode()->RenderContextDestroyed( &m_RenderContext ); // TODO Make BBCamera child of a simpler BBRoot
}

void BBCamera::Trigger(double)
{ 
...
void BBCamera::SetControl( const std::string & control, const std::string & channel )
{
...
*/
vector3 BBCamera::GetPosition( ) const 
{
...
*/
quaternion BBCamera::GetOrientation() const
{
    return m_TransformMatrix.get_quaternion();
}

void BBCamera::SetDebugShape( const char* shapeNodePath )
{
...
void BBCamera::SetSphereCuller( const sphere& cullSphere )
{
    m_CullMode = SPHERE;
...
void BBCamera::SetFrustumCuller()
{
    m_CullMode = FRUSTUM;
}

/**
    Uses BBCullingMgr to perform a cull of game objects, and 
    calls Manipulate, on every object not culled.
    Note that culls aren't perfect!  They may include
    objects outside the cull zone.
*/

void BBCamera::ManipulateVisibleElements()
{
    ...
    for( i = elements.Size(); i > 0; --i )
        Manipulate( *((BBRoot*)(elements[i-1]->GetPtr() )) );
}
...
void BBCamera::Attach( nSceneServer& sceneServer, uint& frameId )
{
    if( m_RenderContext.IsValid() )
    {
...
nVisibilityVisitor* BBCamera::GetVisibilityVisitor( nVisibilityVisitor::VisibleElements& m_Elements ) const
{
    switch( m_CullMode )
    {
    case SPHERE:
...
*/
void BBCamera::Manipulate(nRoot& obj) 
{
    ...
    n_assert( obj.IsA(kernelServer->FindClass( "bbroot" ) ) );
    pCmd->In()->SetO( &obj );
