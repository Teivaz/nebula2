...
nNebulaScriptClass(BBFreeCamera, "bbcamera");

//------------------------------------------------------------------------------
BBFreeCamera::BBFreeCamera() :
    m_Mode(NONE),
    m_MoveSpeed(1.0f),
    m_RotateSpeed( 0.5f )
{
...
*/
void BBFreeCamera::SetPosition(float x, float y, float z)
{
...
void BBFreeCamera::SetOrientation(const quaternion& quat)
{
...
/**
    ...
    button is held down.
*/
void BBFreeCamera::SetSpinMode(SpinMode mode)
{
    m_Mode = mode;
}
...
*/
void BBFreeCamera::Trigger(double deltaTime)
{
...
void BBFreeCamera::MoveBy( const vector3& absDelta )
{
