...
/**
    ...
    @classinfo
    A CCFreeCamera keeps track of its position and orientation and 
...
{
    CCFreeCamera* self = (CCFreeCamera*)slf;
    float x = cmd->In()->GetF();
    float y = cmd->In()->GetF();
    float z = cmd->In()->GetF();
...
n_setspinmode(void* slf, nCmd* cmd)
{
	...
	if(modeStr=="NONE")
	{
        self->SetSpinMode( CCFreeCamera::NONE );
	}
    else if(modeStr=="DRAG")
	{
		self->SetSpinMode( CCFreeCamera::DRAG );
	}
	else if(modeStr=="ROTATE")
	{
		self->SetSpinMode( CCFreeCamera::ROTATE );
	}
    else 
...
*/
bool
CCFreeCamera::SaveCmds(nPersistServer* ps)
{
