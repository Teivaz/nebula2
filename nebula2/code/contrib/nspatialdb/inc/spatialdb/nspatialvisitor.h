...
public:
    ...
    // process an element that is known to be in the region (i.e., don't do a clip check)
    virtual void Visit(nSpatialElement *visitee, int recursiondepth=0);

    /** Check if an element is in the region.
        Returns true if the element is in the region, false if the element is not in the region.
        Note that elements partially in the region may be rejected or accepted, depending on the
        visitor.  Check the docs for that visitor!
